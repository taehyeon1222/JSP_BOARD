...
댓글 작성 및 수정시 빈칸 혹은 공백으로 된 경우 “내용은 필수 항목입니다.”의 메시지 보여주기
 ...
 #### 추천 기능
추천은로그인 유저만 가능하며 중복되지 않도록 한번만 추천가능 하며 한번 더 누를경우 추천수 하락
...
<img src="https://github.com/taehyeon1222/JSP_BOARD/assets/129807676/9525fd3c-927e-4bf7-8ef4-c25d35da7235.JPG" width="100%" height="500">
...
이전 JPA 프로젝트를 통해 MVC 패턴이나 CRUD 작업에 익숙해져서 기존 프로젝트보다 수월하게 프로젝트를 진행할 수 있었습니다. 또한, 이전에 만든 게시판 형태의 프로젝트를 다시 한번 보았을 때 부족했던 부분을 발견할 수 있었습니다.

> 반성한 부분

JPA와 다르게 마이바티스에서는 SQL 공부의 중요성을 느꼈습니다. 예를 들면 JPA에서는 자동으로 쿼리를 생성해 주기 때문에 간단한 쿼리는 간단하게 구현이 되었지만, 마이바티스에서는 간단한 쿼리도 직접 제작해야 했습니다. 이때 쿼리를 만들고 공부하는 과정이 즐거웠으며, 쿼리 공부가 부족함을 느꼈습니다. 레스트풀하게 프로그래밍하지 못한 점과 초반 설계 단계에서 실서비스를 목표로 진행하였던 AWS로 배포는 시도해보지 못한 부분이 가장 아쉬웠습니다.
